...
const App: React.FC = () => {
  ...
  const handleUpdateModule = (updatedModule: StudyModule) => {
    if (plan) {
      const updatedModules = plan.modules.map(m =>
        m.id === updatedModule.id ? updatedModule : m
      );
      setPlan({ ...plan, modules: updatedModules });
      // Update active module ref if it's the one being updated
      if (activeModule && activeModule.id === updatedModule.id) {
  ...
  return (
    <div className="min-h-screen bg-[#fcfdfe] text-slate-900 font-sans selection:bg-indigo-100 selection:text-indigo-700 overflow-x-hidden">
      ...
      <main className="pb-20">
        ...
        {state === AppState.STUDYING && activeModule && plan && (
          <ModuleDetailView
            overallTopic={plan.topic}
            module={activeModule}
